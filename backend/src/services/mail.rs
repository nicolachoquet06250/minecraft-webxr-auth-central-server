...
#[derive(Clone)]
pub struct MailService { config: Option<MailConfig> }
...
#[derive(Debug, Clone)]
pub enum MailKind { Contact, Support }

struct InlineImage { content_id: String, content_type: ContentType, bytes: Vec<u8> }
...
impl MailService {
    pub fn from_env() -> Self {
        ...
        Self { config: Some(MailConfig { smtp_host, smtp_port, smtp_username, smtp_password, smtp_from, contact_to, support_to, starttls }) }
    }
    ...
    async fn send_message(&self, email: Message) -> Result<()> {
        ...
        let mailer = if config.starttls { AsyncSmtpTransport::<Tokio1Executor>::relay(&config.smtp_host).context("Failed to create STARTTLS SMTP relay")? } else { AsyncSmtpTransport::<Tokio1Executor>::builder_dangerous(&config.smtp_host) }.port(config.smtp_port).credentials(credentials).build();
