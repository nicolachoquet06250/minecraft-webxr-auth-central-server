...
pub async fn get_user_active_avatar(
    Extension(_claims): Extension<Claims>,
    State(state): State<Arc<AppState>>,
    Path(user_id): Path<String>,
) -> Result<Json<ActiveAvatarResponse>, StatusCode> {
    User::find_by_id(user_id.clone())
        .one(&state.db)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
...
async fn notify_avatar_created(
    state: &Arc<AppState>,
    user_id: &str,
    saved_avatar: &avatar::Model,
    ...
    if let Err(error) = state
        .mail_service
        .send_avatar_created_email(&current_user.email, &current_user.username, &saved_avatar.name, &preview_image_data_url)
        .await
    {
...
async fn notify_avatar_updated(
    state: &Arc<AppState>,
    user_id: &str,
    saved_avatar: &avatar::Model,
    preview_image_data_url: Option<&str>,
    ...
    if let Err(error) = state
        .mail_service
        .send_avatar_updated_email(&current_user.email, &current_user.username, &saved_avatar.name, &preview_image_data_url)
        .await
    {
