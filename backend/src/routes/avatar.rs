...
async fn notify_avatar_created(state: &Arc<AppState>, user_id: &str, saved_avatar: &avatar::Model) {
    let Ok(Some(current_user)) = User::find_by_id(user_id.to_string()).one(&state.db).await else {
        tracing::warn!(user_id, "avatar created mail skipped: user not found");
        return;
    };
    let svg = full_avatar_svg_from_texture_data(&saved_avatar.texture_data)
        .unwrap_or_else(|_| svg_from_texture_data(&saved_avatar.texture_data).unwrap_or_else(|_| empty_svg()));
    if let Err(error) = state.mail_service.send_avatar_created_email(&current_user.email, &current_user.username, &saved_avatar.name, &svg).await {
        tracing::warn!(?error, avatar_id = %saved_avatar.id, "avatar created mail could not be sent");
    }
}

async fn notify_avatar_updated(state: &Arc<AppState>, user_id: &str, saved_avatar: &avatar::Model) {
    let Ok(Some(current_user)) = User::find_by_id(user_id.to_string()).one(&state.db).await else {
        tracing::warn!(user_id, "avatar updated mail skipped: user not found");
        return;
    };
    let svg = full_avatar_svg_from_texture_data(&saved_avatar.texture_data)
        .unwrap_or_else(|_| svg_from_texture_data(&saved_avatar.texture_data).unwrap_or_else(|_| empty_svg()));
    if let Err(error) = state.mail_service.send_avatar_updated_email(&current_user.email, &current_user.username, &saved_avatar.name, &svg).await {
        tracing::warn!(?error, avatar_id = %saved_avatar.id, "avatar updated mail could not be sent");
    }
}
...
fn full_avatar_svg_from_texture_data(texture_data: &str) -> Result<String, StatusCode> {
    ...
    rects.push_str(&avatar_face_rects(&data, &colors, "head", 32.0, 0.0, 32.0, 32.0)?);
    rects.push_str(&avatar_face_rects(&data, &colors, "torso", 32.0, 32.0, 32.0, 48.0)?);
    rects.push_str(&avatar_face_rects(&data, &colors, "rightArm", 16.0, 32.0, 16.0, 48.0)?);
    rects.push_str(&avatar_face_rects(&data, &colors, "leftArm", 64.0, 32.0, 16.0, 48.0)?);
    rects.push_str(&avatar_face_rects(&data, &colors, "rightLeg", 32.0, 80.0, 16.0, 48.0)?);
    rects.push_str(&avatar_face_rects(&data, &colors, "leftLeg", 48.0, 80.0, 16.0, 48.0)?);
    Ok(format!(r#"<svg xmlns="http://www.w3.org/2000/svg" width="144" height="192" viewBox="0 0 96 128" shape-rendering="crispEdges" style="display:block;margin:auto;max-width:144px;background:transparent;">{}</svg>"#, rects))
}

fn avatar_face_rects(data: &Value, colors: &HashMap<String, [f64; 4]>, part: &str, origin_x: f64, origin_y: f64, target_width: f64, target_height: f64) -> Result<String, StatusCode> {
    let face = data.get("parts").and_then(|parts| parts.get(part)).and_then(|part| part.get("front")).ok_or(StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    let cell_width = target_width / width as f64;
    let cell_height = target_height / height as f64;
    let mut rects = String::new();
    for (y, row) in matrix.iter().enumerate() {
        let row = row.as_str().ok_or(StatusCode::INTERNAL_SERVER_ERROR)?;
        for (x, key) in row.chars().enumerate() {
            let Some([r, g, b, a]) = colors.get(&key.to_string()).copied() else { continue };
            if a <= 0.0 { continue; }
            ...
            rects.push_str(&format!(r#"<rect x="{:.3}" y="{:.3}" width="{:.3}" height="{:.3}" fill="rgb({},{},{})" fill-opacity="{}"/>"#, origin_x + x as f64 * cell_width, origin_y + y as f64 * cell_height, cell_width, cell_height, r, g, b, a.clamp(0.0, 1.0)));
        }
    }
    Ok(rects)
}
...
fn steve_head_matrix() -> Vec<String> {
    vec!["FFFFFFFF".to_string(), "FAAKKKAF".to_string(), "ABBJBBGK".to_string(), "NBNNGBII".to_string(), "JCENJEMJ".to_string(), "IBNLLNGD".to_string(), "IIPHHKDD".to_string(), "HHOPPODD".to_string()]
}

fn alex_head_matrix() -> Vec<String> {
    vec!["BBBBBBBB".to_string(), "BCCDDCBB".to_string(), "BCCHHCCB".to_string(), "BGGHHGGB".to_string(), "GJKHHKJG".to_string(), "GGGHGGGG".to_string(), "GGGLLGGG".to_string(), "GGGGGGGG".to_string()]
}
