...
pub async fn get_user_profile_pic_svg(Extension(_claims): Extension<Claims>, State(state): State<Arc<AppState>>, Path(user_id): Path<String>) -> Result<Response, StatusCode> { profile_pic_response(&state, &user_id).await }
...
pub async fn select_avatar(Extension(claims): Extension<Claims>, State(state): State<Arc<AppState>>, Path(avatar_id): Path<String>) -> Result<StatusCode, StatusCode> {
    ...
    let avatars = Avatar::find().filter(avatar::Column::UserId.eq(claims.sub)).all(&state.db).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    for item in avatars { let should_activate = item.id == owned.id; let mut active: avatar::ActiveModel = item.into(); active.is_active = Set(should_activate); active.updated_at = Set(chrono::Utc::now().naive_utc()); active.update(&state.db).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?; }
...
pub async fn clear_active_avatar(Extension(claims): Extension<Claims>, State(state): State<Arc<AppState>>) -> Result<StatusCode, StatusCode> {
    let avatars = Avatar::find().filter(avatar::Column::UserId.eq(claims.sub)).all(&state.db).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    for item in avatars { let mut active: avatar::ActiveModel = item.into(); active.is_active = Set(false); active.updated_at = Set(chrono::Utc::now().naive_utc()); active.update(&state.db).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?; }
    Ok(StatusCode::NO_CONTENT)
}
...
fn normalize_base_kind(value: &str) -> String { match value { "steve" => "steve".to_string(), "alex" => "alex".to_string(), _ => "custom".to_string() } }
fn to_response(model: avatar::Model) -> AvatarResponse { AvatarResponse { id: model.id, name: model.name, base_kind: model.base_kind, is_active: model.is_active, texture_data: serde_json::from_str::<Value>(&model.texture_data).unwrap_or(Value::Null), created_at: model.created_at.to_string(), updated_at: model.updated_at.to_string() } }
...
fn svg_from_texture_data(texture_data: &str) -> Result<String, StatusCode> {
    ...
    for (key, value) in palette { let channel_values = value.as_array().ok_or(StatusCode::INTERNAL_SERVER_ERROR)?; if channel_values.len() != 4 { return Err(StatusCode::INTERNAL_SERVER_ERROR); } let mut channels = [0.0_f64; 4]; for (index, channel) in channel_values.iter().enumerate() { channels[index] = channel.as_f64().ok_or(StatusCode::INTERNAL_SERVER_ERROR)?; } colors.insert(key.clone(), channels); }
...
fn default_profile_pic_svg(avatar: &str) -> String { match avatar { "steve" => build_svg(8, 8, &steve_head_matrix(), steve_head_color).unwrap_or_else(|_| empty_svg()), _ => build_svg(8, 8, &alex_head_matrix(), alex_head_color).unwrap_or_else(|_| empty_svg()) } }

fn build_svg<F>(width: usize, height: usize, matrix: &[String], color_for: F) -> Result<String, StatusCode> where F: Fn(char) -> Option<[f64; 4]> {
    let cell = 12; let svg_width = width * cell; let svg_height = height * cell; let mut rects = String::new();
    for (y, row) in matrix.iter().enumerate() { for (x, key) in row.chars().enumerate() { let Some([r, g, b, a]) = color_for(key) else { continue }; if a <= 0.0 { continue; } let r = (r * 255.0).round().clamp(0.0, 255.0) as u8; let g = (g * 255.0).round().clamp(0.0, 255.0) as u8; let b = (b * 255.0).round().clamp(0.0, 255.0) as u8; rects.push_str(&format!(r#"<rect x="{}" y="{}" width="{}" height="{}" fill="rgb({},{},{})" fill-opacity="{}"/>"#, x * cell, y * cell, cell, cell, r, g, b, a.clamp(0.0, 1.0))); } }
