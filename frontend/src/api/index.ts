...
export const userApi = { getProfile: (): ApiResponse<User> => request<User>('/users/me'), getUserById: (id: string): ApiResponse<User> => request<User>(`/users/${id}`), updateProfile: (data: Partial<User>): ApiResponse<User> => request<User>('/users/me', { method: 'PUT', body: jsonBody(data) }), unlinkDiscord: (): ApiResponse<User> => request<User>('/users/me/discord', { method: 'DELETE' }), deleteAccount: (): ApiResponse<null> => request<null>('/users/me', { method: 'DELETE' }), requestPasswordChangeCode: (data: PasswordChangeCodeRequest): ApiResponse<PasswordChangeCodeResponse> => request<PasswordChangeCodeResponse>('/users/me/password/change-code', { method: 'POST', body: jsonBody(data) }), confirmPasswordChange: (data: PasswordChangeConfirmRequest): ApiResponse<PasswordChangedResponse> => request<PasswordChangedResponse>('/users/me/password', { method: 'PUT', body: jsonBody(data) }) }
