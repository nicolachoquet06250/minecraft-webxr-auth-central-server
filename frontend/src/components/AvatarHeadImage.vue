<template>
  <img :src="svgUrl" :alt="avatar || 'avatar'" class="avatar-head-image" />
</template>
...
const texture = computed<TextureMatrix>(() => props.avatar === 'steve' ? steveModelTextures.head.front : alexModelTextures.head.front)
...
const svgUrl = computed(() => {
  ...
  for (let y = 0; y < face.matrix.length; y += 1) {
    ...
    for (let x = 0; x < row.length; x += 1) {
      ...
      if (!color) continue
      const fill = `rgb(${Math.round(color[0] * 255)},${Math.round(color[1] * 255)},${Math.round(color[2] * 255)})`
